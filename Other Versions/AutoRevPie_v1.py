...
class AutoPause:
    def __init__ (self):
        ...
        self.maxCalls = 0.75 # ratio of Reps in Queue to use
                             # for second callLimit, after lowering bids
    ...
    def autoPause(self, repsAvail, isPaused):
        ...
        and datetime.datetime.today().weekday() < 5):
                ...
                printToLog('\n\nSwitched to 11am campaign\n'
                           , "")
                # wait until 11:02
                #target = datetime.datetime.strptime("11:02", "%H:%M")
                #sleep_until(target)
                waiting(60)
        ...
        and datetime.datetime.today().weekday() < 5):
                ...
                printToLog('\n\nSwitched to 7pm campaign\n'
                           , "")
                # wait until 19:02
                #target = datetime.datetime.strptime("19:02", "%H:%M")
                #sleep_until(target)
                waiting(60)
...
class autoBidAdjust:
    ...
    def copyBids(self):
        ...
        # for index, item in enumerate(self.sourceIDs[0])
        for item in range(len(self.sourceIDs[0])):
            customBid = 'customBid_' + item
            try:
                self.customBids.append(
                        browser.find_element_by_id(
                                customBid).get_attribute("value"))
        ...
        for index, item in enumerate(sourceIDs):
            ...
            WebDriverWait(browser, 15).until(
                    expected_conditions.presence_of_element_located(
                            (By.ID,'bidAdjustmentsTable')))
            try:
                text_area = browser.find_element_by_id(customBid)
                text_area.send_keys(Keys.CONTROL, 'a')
                waiting(1)
                text_area.send_keys(""+customBids[index]+"")
                waiting(1)
                browser.execute_script("makeCustomBidAdjustment("+item+")")
            except:pass # will pass if the sourceID is not available
        ...
        for index, item in enumerate(sourceIDs):
            ...
            WebDriverWait(browser, 15).until(
                    expected_conditions.presence_of_element_located(
                            (By.ID,'bidAdjustmentsTable')))
            try:
                if int(float(customBids[index])) != 0.01:
                        text_area = browser.find_element_by_id(customBid)
                        text_area.send_keys(Keys.CONTROL, 'a')
                        waiting(1)
                        text_area.send_keys(""+(int(float(customBids[index])) - 0.10)+"")
                        browser.execute_script("makeCustomBidAdjustment("+item+")")
            except:pass # pass if the sourceID is not available
        return(self.sourceIDs[0], self.customBids)
    ...
    def raiseBids(self):
        ...
        try:
            sourceIDs = self.sourceIDs[0]
            customBids = self.customBids
        except:pass # silent fail if no previous values stored, i.e lowerBids() was not called
        for index, item in enumerate(sourceIDs):
            customBid = 'customBid_' + item
            WebDriverWait(browser, 15).until(
                    expected_conditions.presence_of_element_located(
