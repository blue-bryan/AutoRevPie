...
while loop:
    loop = checkBrowser()
    try:
        AutoPauser.autoPause(RepCounter.repCount)       
    except KeyboardInterrupt:
        print("\n")
        sys.exit(1)
    except Exception as err:
    ...
    if datetime.datetime.now().minute % 5 == 0:
        ...
        if datetime.datetime.now().minute % 10 == 0:
            try:
                ...
                waiting(1)
                CampaignStats.getStatsPage(AutoPauser.currentCampaign
                                            , AutoPauser.campaigns[AutoPauser.currentCampaign][0])
                print(CampaignStats.getStatsUpdate(AutoPauser.currentCampaign
                                                    , AutoPauser.campaigns[AutoPauser.currentCampaign][0]), '\n')
                _wait = True
                while _wait:
